...
def  dbconn_subdomain_db():
    conn = MySQLdb.Connect(host='127.0.0.1',user='root',passwd='root',db='pentest',port=3306,charset='utf8')
    return conn
                ...
                try:
                    ...
                    if answers:
                        ...
                        if (not self.ignore_intranet) or (not DNSBrute.is_intranet(answers[0].address)):
                            ...
                            sql = '''insert into subdomain(subdomain,ips,target,scantools,scantime) values(%s,%s,%s,%s,null)'''
                            cur.execute(sql,(cur_sub_domain.ljust(30),ips,self.target,'subdomainburte'))
                            cur.close()
                            self.conn.commit()
                            self.lock.release()
                            try:
